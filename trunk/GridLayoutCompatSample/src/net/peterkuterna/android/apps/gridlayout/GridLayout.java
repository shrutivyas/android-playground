 ...
 * viewing area into <em>cells</em>. Throughout the API, grid lines are
 * referenced by grid <em>indices</em>. A grid with {@code N} columns has
 * {@code N + 1} grid indices that run from {@code 0} through {@code N}
 * inclusive. Regardless of how GridLayout is configured, grid index {@code 0}
 * is fixed to the leading edge of the container and grid index {@code N} is
 * fixed to its trailing edge (after padding is taken into account).
 * 
 * <h4>Row and Column Specs</h4>
 * 
 * Children occupy one or more contiguous cells, as defined by their
 * {@link GridLayout.LayoutParams#rowSpec rowSpec} and
 * {@link GridLayout.LayoutParams#columnSpec columnSpec} layout parameters. Each
 * spec defines the set of rows or columns that are to be occupied; and how
 * children should be aligned within the resulting group of cells. Although
 * cells do not normally overlap in a GridLayout, GridLayout does not prevent
 * children being defined to occupy the same cell or group of cells. In this
 * case however, there is no guarantee that children will not themselves overlap
 * after the layout operation completes.
 * 
 * <h4>Default Cell Assignment</h4>
 * 
 * If a child does not specify the row and column indices of the cell it wishes
 * to occupy, GridLayout assigns cell locations automatically using its:
 ...
 * 
 * layout parameters. When the {@link GridLayout#setUseDefaultMargins(boolean)
 * useDefaultMargins} property is set, default margins around children are
 * automatically allocated based on the prevailing UI style guide for the
 * platform. Each of the margins so defined may be independently overridden by
 * an assignment to the appropriate layout parameter. Default values will
 * generally produce a reasonable spacing between components but values may
 ...
 * {@link LayoutParams#setGravity(int) gravity} property of the child's layout
 * parameters). If alignment was defined along a given axis then the component
 * is taken as <em>flexible</em> in that direction. If no alignment was set, the
 * component is instead assumed to be <em>inflexible</em>.
 * <p>
 * Multiple components in the same row or column group are considered to act in
 * <em>parallel</em>. Such a group is flexible only if <em>all</em> of the
 * components within it are flexible. Row and column groups that sit either side
 * of a common boundary are instead considered to act in <em>series</em>. The
 * composite group made of these two elements is flexible if <em>one</em> of its
 * elements is flexible.
 * <p>
 * To make a column stretch, make sure all of the components inside it define a
 * gravity. To prevent a column from stretching, ensure that one of the
 * components in the column does not define a gravity.
 * <p>
 * When the principle of flexibility does not provide complete disambiguation,
 * GridLayout's algorithms favour rows and columns that are closer to its
 * <em>right</em> and <em>bottom</em> edges.
 * 
 * <h5>Limitations</h5>
 * 
 * GridLayout does not provide support for the principle of <em>weight</em>, as
 * defined in {@link LinearLayout.LayoutParams#weight}. In general, it is not
 * therefore possible to configure a GridLayout to distribute excess space in
 * non-trivial proportions between multiple rows or columns.
 * <p>
 ...
 * {@link #CENTER} alignment (or {@link LayoutParams#setGravity(int) gravity}).
 * For complete control over excess space distribution in a row or column; use a
 * {@link LinearLayout} subview to hold the components in the associated cell
 * group. When using either of these techniques, bear in mind that cell groups
 * may be defined to overlap.
 * <p>
 * See {@link GridLayout.LayoutParams} for a full description of the layout
 * parameters used by GridLayout.
 * 
...
public class GridLayout extends ViewGroup {
	...
	public GridLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		defaultGap = context.getResources().getDimensionPixelOffset(
				R.dimen.default_gap);
	...
	private int getDefaultMargin(View c, boolean isAtEdge, boolean horizontal,
			boolean leading) {
		return isAtEdge ? DEFAULT_CONTAINER_MARGIN : getDefaultMargin(c,
				horizontal, leading);
	}
	...
	int getMargin1(View view, boolean horizontal, boolean leading) {
		...
		return margin == UNDEFINED ? getDefaultMarginValue(view, lp,
	...
	private int getTotalMargin(View child, boolean horizontal) {
		return getMargin(child, horizontal, true)
				+ getMargin(child, horizontal, false);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (DEBUG) {
			...
			for (int i = 0; i < getChildCount(); i++) {
				View c = getChildAt(i);
				drawRect(canvas, c.getLeft() - getMargin1(c, true, true),
						c.getTop() - getMargin1(c, false, true), c.getRight()
								+ getMargin1(c, true, false), c.getBottom()
								+ getMargin1(c, false, false), paint);
			}
		}
	}
	...
	private void measureChildrenWithMargins(int widthSpec, int heightSpec,
		...
		for (int i = 0, N = getChildCount(); i < N; i++) {
			...
			if (firstPass) {
				measureChildWithMargins2(c, widthSpec, heightSpec, lp.width,
						lp.height);
			} else {
				Spec spec = (orientation == HORIZONTAL) ? lp.columnSpec
						: lp.rowSpec;
				if (spec.alignment == FILL) {
					Interval span = spec.span;
					Axis axis = (orientation == HORIZONTAL) ? horizontalAxis
							: verticalAxis;
	...
	protected void onMeasure(int widthSpec, int heightSpec) {
		...
		setMeasuredDimension(resolveSizeAndState(measuredWidth, widthSpec, 0),
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int i = 0, N = getChildCount(); i < N; i++) {
			...
			// its cell group.
			// noinspection NullableProblems
			int c2ax = protect(hAlign.getAlignmentValue(null, cellWidth
					- colBounds.size(true)));
			// noinspection NullableProblems
			int c2ay = protect(vAlign.getAlignmentValue(null, cellHeight
					- rowBounds.size(true)));
	...
	final class Axis {
		...
		private int getMaxIndex() {
			if (maxIndex == UNDEFINED) {
				maxIndex = max(0, calculateMaxIndex()); // use zero when there
														// are no children
			}
			return maxIndex;
		}
		...
		private PackedMap<Spec, Bounds> createGroupBounds() {
			...
			for (int i = 0, N = getChildCount(); i < N; i++) {
				...
				LayoutParams lp = getLayoutParams(c);
				Spec spec = horizontal ? lp.columnSpec : lp.rowSpec;
				Bounds bounds = getAlignment(spec.alignment, horizontal)
						.getBounds();
		...
		private void solve(Arc[] arcs, int[] locations) {
			...
			int N = getCount() + 1; // The number of vertices is the number of
									// columns/rows + 1.
		...
		private void computeMargins(boolean leading) {
			...
			for (int i = 0, N = getChildCount(); i < N; i++) {
				...
				margins[index] = max(margins[index],
						getMargin1(c, horizontal, leading));
			}
		}
	...
	public static class LayoutParams extends MarginLayoutParams {
		...
		public void setGravity(int gravity) {
			...
			columnSpec = columnSpec.copyWriteAlignment(getAlignment(gravity,
					true));
		}
	...
	static class Bounds {
		...
		protected void reset() {
			...
			flexibility = CAN_STRETCH; // from the above, we're flexible when
										// empty
		}
	...
	public static class Spec {
		...
		final int getFlexibility() {
			return (alignment == UNDEFINED_ALIGNMENT) ? INFLEXIBLE
					: CAN_STRETCH;
		}
